...
trial2show = 1;
load([input_dir filesep monkey_dir '_' the_sess '_tr' num2str(trial2show) '_ecog.mat'],'data','comment','sampleRate','startViconNSP','endViconNSP')
load([input_dir filesep monkey_dir '_' the_sess '_tr' num2str(trial2show) '_vicon.mat'],'kinematic','analog')
...
for ch = 10%:32
    ...
    for kk = 1:2
        ...
        amp = abs(s);
        % ampNorm = amp./repmat(mean(amp,2),1,size(amp,2));
...
%         dataAmp = imgaussfilt(dataAmp,sigma);
    %     dataAmp = imgaussfilt(dataAmp,sigma);
        hold on
    ...
    %     colorbar
        plot(tsViconCorr,(x-200)/200*50-50,'-r','linewidth',2)
        set(gca,'xlim',time_range,'ylim',[-50 freq_range(2)],...
            'clim',c_range)
    %     set(gca,'xlim',time_range,'ylim',[-50 freq_range(2)],...
    %         'clim',[min(dataAmp(:)),prctile(dataAmp(:),98)])
        set(gca,'ydir','normal')
    ...
    if isSave
        saveas(gcf,[save_dir filesep 'Trial ' num2str(trial2show) ' - Activation_chan' num2str(ch) '.fig'])
        saveas(gcf,[save_dir filesep 'Trial ' num2str(trial2show) ' - Activation_chan' num2str(ch) '.png'])
    end
end
